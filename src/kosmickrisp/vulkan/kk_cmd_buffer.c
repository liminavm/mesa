                          ...
                          VkPipelineBindPoint bind_point)
{
   ...
   if (kk_limina_slimroot()) {
      uint32_t used = 0;
      if (bind_point == VK_PIPELINE_BIND_POINT_GRAPHICS) {
         for (unsigned s = 0; s <= MESA_SHADER_FRAGMENT; s++) {
            struct kk_shader *shader = cmd->state.shaders[s];
            if (shader == NULL)
               continue;
            if (shader->info.root_used_size == 0) {
               used = 0; /* unknown bound -> full size */
               break;
            }
            used = MAX2(used, shader->info.root_used_size);
         }
      } else {
         struct kk_shader *shader = cmd->state.shaders[MESA_SHADER_COMPUTE];
         if (shader != NULL)
            used = shader->info.root_used_size;
      }
      if (used != 0)
         size = MIN2(align(used, 8u), (uint32_t)sizeof(*root));
   }

   struct kk_ptr root_gpu = kk_pool_upload(cmd, root, size, 8u);
